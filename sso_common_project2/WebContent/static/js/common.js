	...
	$.fn.extend({
		browser: function(options) {
			var settings = {
					...
					isUpload: true,
					browserUrl: emall.base + "/file/browser.jhtml",
					uploadUrl: emall.base + "/file/upload.jhtml",
					callback: null
				};
				$.extend(settings, options);
						...
						function createBrowserList(path, data) {
							...
							$.each(data, function(i, fileInfo) {
								...
								if (fileInfo.isDirectory) {
									iconUrl = emall.base + "/static/images/folder_icon.gif";
									title = fileInfo.name;
									...
									iconUrl = emall.base + "/static/images/file_icon.gif";
									title = fileInfo.name + " (" + Math.ceil(fileInfo.size / 1024) + "KB, " + new Date(fileInfo.lastModified).toLocaleString() + ")";
								}
